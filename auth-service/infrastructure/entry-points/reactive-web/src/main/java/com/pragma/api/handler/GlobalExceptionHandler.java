...
import com.pragma.api.helper.ResponseHelper;
    ...
    public Mono<Void> handle(ServerWebExchange exchange, Throwable ex) {
        ...
        switch (ex) {
            ...
            case ServerWebInputException inputEx -> {
                status = HttpStatus.BAD_REQUEST;
                String param = Optional.ofNullable(inputEx.getMethodParameter()).map(MethodParameter::getParameterName).orElse("Unknown");
            ...
            case ResponseStatusException resEx -> {
                ...
                body = new ApiResponseDto<>(resEx.getReason() != null ? resEx.getReason() : "Error en la petición", "Ruta no encontrada", status);
            }
